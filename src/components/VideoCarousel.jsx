...
import { FaPlay, FaChevronLeft, FaChevronRight, FaClock, FaUser, FaTag } from "react-icons/fa";
        ...
        >
          ...
          >
            ...
            <p className="mt-4 text-gray-600 max-w-2xl mx-auto">
              Explore our collection of free educational videos to boost your learning journey
            </p>
          </motion.div>
                ...
                <div key={video.id} className="keen-slider__slide">
                  ...
                  >
                    <div className="relative group">
                      ...
                      {/* Duration badge */}
                      <div className="absolute top-3 left-3 bg-black bg-opacity-75 text-white text-xs px-3 py-1 rounded-full flex items-center">
                        <FaClock className="mr-1" /> {video.duration}
                      </div>
                    </div>

                    <div className="p-5">
                      <h3 className="font-bold text-lg mb-2 line-clamp-2 text-gray-800">{video.title}</h3>
                      ...
                      <div className="flex items-center mt-3 pt-3 border-t border-gray-100">
                        <div className="flex">
                          {[...Array(5)].map((_, i) => (
                            <svg 
                              key={i} 
                              className={`w-4 h-4 ${i < Math.floor(Number(video.rating)) ? "text-yellow-400" : "text-gray-300"}`}
                              fill="currentColor" 
                              viewBox="0 0 20 20"
                            >
                        ...
                        <span className="text-yellow-500 ml-1 font-medium">{video.rating}</span>
            ...
            {loaded && instanceRef.current && (
              <>
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                  className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 bg-white text-green-600 p-3 rounded-full shadow-lg z-10 focus:outline-none"
                ...
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                  className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 bg-white text-green-600 p-3 rounded-full shadow-lg z-10 focus:outline-none"
            ...
            <div className="flex justify-center mt-8">
              {[...Array(instanceRef.current.track.details.slides.length).keys()].map((idx) => (
                <button
                  ...
                  className={`mx-1 w-3 h-3 rounded-full focus:outline-none transition-colors duration-200 ${
          ...
          >
            <motion.a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center px-6 py-3 bg-green-600 text-white font-medium rounded-full shadow-lg hover:bg-green-700 transition-colors"
